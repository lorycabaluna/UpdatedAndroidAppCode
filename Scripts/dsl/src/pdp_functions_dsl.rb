module PDPFunctionsDSL
  ...
  def search_for_available_store(zipCode,xCoordinate,yCoordinate,bopsTrueOrFalse)
    ...
    #page_scroll_down(0.50,0.30)
      ...
      puts "CHECK STORES FOR PICKUP is clicked"
        while exists{bops_hops_puas_button} && @count < 5
          ...
          if exists{bops_zipcode_search_field}
            @cnt = 0
              ...
              if exists{bops_select_store_button}
                ...
                while ((!exists{bops_select_store_button} && exists{bops_select_callstore_button}) && @cnt < 4 )
                  tap_store_location(xCoordinate,yCoordinate)
                  #C  sleep 3
                  yCoordinate += 286
                  tap_store_location(xCoordinate,yCoordinate)
  ...
  def search_for_available_store_2(zipCode,xCoordinate,yCoordinate,bopsTrueOrFalse)
    @count = 0
    ...
    puts "CHECK STORES FOR PICKUP is clicked"
     while exists{bops_hops_puas_button} && @count < 5
      ...
      if exists{bops_zipcode_search_field}
        ...
        bops_pay_now_button.click
        #C  sleep 15
        expect(exists{cart_header_checkout_button}).to eq(true)
        cart_header_checkout_button.click
        #C  sleep 30


      elsif exists{cart_header_checkout_button}
        expect(exists{cart_header_checkout_button}).to eq(true)
        cart_header_checkout_button.click
  ...
  def enter_new_billing_address(billingFname)
    ...
    hide_keyboard
    enter_text(shipping_email_address, 'bf_ecp1@qagsecomprod.oib.com')
    hide_keyboard
  ...
  def hops_your_details_page
    ...
    if (yourDetails_FirstName.text !='')
    continue_buttonHops.click
      #C  sleep 5
    else
    yourDetails_FirstName.clear
    yourDetails_FirstName.send_keys('Lory')
    yourDetails_LastName.clear
    yourDetails_LastName.send_keys('Cabaluna')
    yourDetails_Email.clear
    yourDetails_Email.send_keys('lorycabaluna@gamestop.com')
    page_scroll_down('0.50','0.30')
  ...
  def edit_hops_your_details_page
    ...
    yourDetails_FirstName.send_keys('Lory')
    yourDetails_LastName.clear
    yourDetails_LastName.send_keys('San')
    yourDetails_Email.clear
    yourDetails_Email.send_keys('lorycabaluna@gamestop.com')
    page_scroll_down('0.50','0.30')
    yourDetails_PhoneNumber.clear
         ...
         if exists{gc_or_tc_label}
          ...
          if payment_type_cc_or_paypal != '' && payment_type_cc_or_paypal != 'gc'
            ...
            when 'cc'
                ...
                if exists{shipping_first_name} # exists{add_edit_address}
                  enter_new_billing_address(billingFname) #comment
                  checkout_save_button.click
                  #C  sleep 10
                else
                  checkout_save_button.click
  ...
  def checkout_page_for_BOPS_PreOrder(promo_code, gift_card_or_trade_card,pur_trade_credit,payment_type_cc_or_paypal,enter_CreditCardType,billingFname)
    ...
    if payment_type_cc_or_paypal != 'paypal'
      ...
      if billingFname =='Reject'
        ...
        page_scroll_down(0.50,0.10)
        expect(readyForPickUpTxtNotification.text).to eq("Please wait for a \"ready for pick up\" notification from us. Thank you!")
        expect(shipGroupHeader.text).to eq("Pick Up At Store Information")
        expect(cart_availability_variants.text).to include("Pre-order")
        page_scroll_down(0.10,0.50)
        continueShoppingConfirmation.click
      end
  ...
  def sku_item_searchOnly(text_input)

    tap_search_icon
    #search_icon.click
    search_field.click
    search_field.send_keys(text_input)
    keypad_search_button_tap
    sleep 10
  end
  ...
  def sku_item_search(sku_number)
    ...
    if exists{itemSearchCategory}
      @count = 0
      while exists{itemSearchCategory} && @count <= 7
        search_field.click
        keypad_search_button_tap
        @count+=1
    ...
    if exists{unable_to_search_item}
       @count = 0
      while exists{unable_to_search_item} && @count <= 7
        search_field.click
        keypad_search_button_tap
        @count+=1
