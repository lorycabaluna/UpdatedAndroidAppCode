...
module SearchFunctionsDSL
  ...
  def select_from_sort_dropdown(text_value)
    ...
    #expected_sort_options = ["Best Matches", "Price Low To High", "Price High to Low", "Product Name A - Z", "Product Name Z - A", "Most Popular", "Top Sellers", "Release Date Oldest to Newest", "Release Date Newest to Oldest", "Most Relevant"]
    ...
    # expected_sort_options = ["Best Matches", "Price Low To High", "Price High to Low", "Product Name A - Z", "Product Name Z - A", "Most Popular", "Top Sellers", "Release Date Oldest to Newest", "Release Date Newest to Oldest", "Most Relevant"]
    # expected_sort_options.each do |search_dd|
    #   if search_dd == text_value
        sort_by_item(text_value).click
        sleep 5
      # end
    # end
  ...
  def filter_category_sublist(itemName,itemType)
    case itemType.upcase
      when 'TEXTVIEW'
        filter_category_textItem(itemName).click
        sleep 5
      when 'CHECKBOX'
        if exists{filter_category_checkBoxItem(itemName)}
          filter_category_checkBoxItem(itemName).click
          sleep 5
        end
      when 'RADIOBUTTON'
        filter_category_radioButtonItem(itemName).click
        sleep 5
