module PURFunctionsDSL
  ...
  def purCardDetailsEnterPIN(pinNumber)
    pur_cardDetails.click
    expect(exists{pur_cardDetails_info}).to eq(true)
    expect(exists{pur_cardDetails_barcode}).to eq(true)
    expect(exists{pur_cardDetails_payAndSaveToPhone}).to eq(true)
    expect(exists{pur_cardDetails_tradeCreditEasyAccess}).to eq(true)
    pur_cardDetails_tradeCreditEasyAccess.click
    sleep 5
    pur_cardDetails_tradeCreditEasyAccessPIN.click
    pur_cardDetails_tradeCreditEasyAccessPIN.send_keys(pinNumber)
    keypad_search_button_tap
    sleep 5

    while !exists{home_tab} do
      press_keycode(4)
    end
    sleep 5

    expect(exists{pur_cardDetails_tradeCreditBalanceHeader}).to eq(true)
    expect(exists{pur_cardDetails_tradeCreditBalanceValue}).to eq(true)
